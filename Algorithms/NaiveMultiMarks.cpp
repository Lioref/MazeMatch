#include "NaiveMultiMarks.h"


using namespace std;

/// empty constructer
NaiveMultiMarks::NaiveMultiMarks() {
    // init coordinates
    this->px = 0;
    this->py = 0;
...
/// upon choice.
void NaiveMultiMarks::moveLeft() {
    this->px--;
    this->lastLastMove = this->lastMove;
    this->lastMove = LEFT;
}

void NaiveMultiMarks::moveRight() {
    this->px++;
    this->lastLastMove = this->lastMove;
    this->lastMove = RIGHT;
}

void NaiveMultiMarks::moveDown() {
    this->py--;
    this->lastLastMove = this->lastMove;
    this->lastMove = DOWN;
}

void NaiveMultiMarks::moveUp() {
    this->py++;
    this->lastLastMove = this->lastMove;
    this->lastMove = UP;
}
...
void NaiveMultiMarks::setBookmark() {
...
void NaiveMultiMarks::postMoveUpdate() {
...
void NaiveMultiMarks::applyMove(Move direction) {
...
int NaiveMultiMarks::posiMod(int num, int div) {
    return ((num % div) + div) % div;
}

/// update the coordinates of visited cells to match new
/// dimension upper bounds
void NaiveMultiMarks::foldVisitedMap() {
...
/// dimension upper bounds
void NaiveMultiMarks::foldWallsSet() {
...
std::map<AbstractAlgorithm::Move, std::tuple<int, int>> NaiveMultiMarks::getPossibleMovePositions() {
...
AbstractAlgorithm::Move NaiveMultiMarks::getLeastVisitedMove(map<AbstractAlgorithm::Move, std::tuple<int, int>> nonWallMoves) {
...
AbstractAlgorithm::Move NaiveMultiMarks::getRandomMove(map<AbstractAlgorithm::Move, std::tuple<int, int>> nonWallMoves) {
    // erase the backtracking move if exists in map
    nonWallMoves.erase(this->oppositeMoves[this->lastMove]);
    assert(nonWallMoves.size() > 0);
...
AbstractAlgorithm::Move NaiveMultiMarks::move() {
    ...
    if (nonWallMoves.size()==1) {
        ...
        Move randMove = getRandomMove(nonWallMoves);
        applyMove(randMove);
        return randMove;
    }
}
...
void NaiveMultiMarks::hitBookmark(int seq) {
...
void NaiveMultiMarks::printWalls() {
...
void NaiveMultiMarks::printVisited() {
...
REGISTER_ALGORITHM(NaiveMultiMarks)
