#include "ShortestPathExploration.h"


/// empty constructer
ShortestPathExploration::ShortestPathExploration() {
    // init coordinates
    this->px = 0;
    this->py = 0;
...
/// upon choice.
void ShortestPathExploration::moveLeft() {
    this->px--;
    this->lastLastMove = this->lastMove;
    this->lastMove = LEFT;
}

void ShortestPathExploration::moveRight() {
    this->px++;
    this->lastLastMove = this->lastMove;
    this->lastMove = RIGHT;
}

void ShortestPathExploration::moveDown() {
    this->py--;
    this->lastLastMove = this->lastMove;
    this->lastMove = DOWN;
}

void ShortestPathExploration::moveUp() {
    this->py++;
    this->lastLastMove = this->lastMove;
    this->lastMove = UP;
}
...
void ShortestPathExploration::setBookmark() {
...
void ShortestPathExploration::postMoveUpdate() {
...
void ShortestPathExploration::applyMove(Move direction) {
...
int ShortestPathExploration::posiMod(int num, int div) {
    return ((num % div) + div) % div;
}

/// update the coordinates of visited cells to match new
/// dimension upper bounds
void ShortestPathExploration::foldVisitedMap() {
...
/// dimension upper bounds
void ShortestPathExploration::foldWallsSet() {
...
std::map<AbstractAlgorithm::Move, std::tuple<int, int>> ShortestPathExploration::getPossibleMovePositions(int x, int y) {
...
AbstractAlgorithm::Move ShortestPathExploration::getLeastVisitedMove(map<AbstractAlgorithm::Move, std::tuple<int, int>> nonWallMoves) {
...
AbstractAlgorithm::Move ShortestPathExploration::getRandomMove(map<AbstractAlgorithm::Move, std::tuple<int, int>> nonWallMoves) {
    // erase the backtracking move if exists in map
    nonWallMoves.erase(this->oppositeMoves[this->lastMove]);
    assert(nonWallMoves.size() > 0);
...
AbstractAlgorithm::Move ShortestPathExploration::move() {
...
void ShortestPathExploration::hitWall() {
...
void ShortestPathExploration::hitBookmark(int seq) {
...
/// move.
void ShortestPathExploration::print() {
...
void ShortestPathExploration::printWalls() {
...
void ShortestPathExploration::printVisited() {
...
tuple<int,int> ShortestPathExploration::getBoundedCoords(int x, int y) {
...
ShortestPathExploration::Move ShortestPathExploration::getFirstMoveFromPathsMap(
...
bool ShortestPathExploration::isWhite(const list<tuple<int,int>>& greyNodes, const set<tuple<int,int>>& blackNodes, const tuple<int,int>& nextMoveCoords) {
...
bool ShortestPathExploration::isGrey(const list<tuple<int,int>>& greyList, const tuple<int,int>& var) {
    return find(greyList.begin(), greyList.end(), var) != greyList.end();
}

/// Utility for BFS algorithm - checks if Cell is in black set
bool ShortestPathExploration::isBlack(const set<tuple<int,int>>& blackNodes, const tuple<int,int>& var) {
    return blackNodes.find(var) != blackNodes.end();
}

REGISTER_ALGORITHM(ShortestPathExploration)
